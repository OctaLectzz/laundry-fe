<template>
  <div>
    <q-form @submit="editKaryawan">
      <q-card style="min-width: 400px">
        <q-card-section class="row items-center q-py-sm">
          <div class="text-h6">Edit Karyawan</div>
          <q-space />
...
const { karyawan } = defineProps(['karyawan'])
...
const editKaryawan = async () => {
  ...
  try {
    await karyawanStore.edit(data.value)
    $q.notify({
      message: 'Berhasil Mengedit Karyawan',
      icon: 'check',
      color: 'positive'
    })
    emits('edited')
    ...
    $q.notify({
      message: error.response.data.message || 'Gagal Mengedit Karyawan',
      icon: 'warning',
      color: 'negative'
    })
  }
