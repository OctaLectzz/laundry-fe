<template>
  <div>
    <q-form @submit="addJenisLayanan">
      <q-card style="min-width: 400px">
        <q-card-section class="row items-center q-py-sm">
          <div class="text-h6">Tambah Jenis Layanan</div>
          <q-space />
...
const addJenisLayanan = async () => {
  ...
  try {
    await jenisLayananStore.create(data.value)
    $q.notify({
      message: 'Berhasil Menambah Jenis Layanan',
      icon: 'check',
      color: 'positive'
    })
    emits('added')
    ...
    $q.notify({
      message: error.response.data.message || 'Gagal Menambah Jenis Layanan',
      icon: 'warning',
      color: 'negative'
    })
  }
